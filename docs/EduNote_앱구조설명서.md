...
| 교무행정AI | `SCHOOL_DOC` | `SchoolDocPanel` | 공문서, 계획서, 연수자료, 보고서 등 10종 문서 생성 |
...
| `TRAINING_MATERIAL` | 연수자료 | 연수 개요 표 → Ⅰ.연수목표 → Ⅱ.필요성 → Ⅲ.핵심내용(표 포함) → Ⅳ.사례와 판단기준(표) → Ⅴ.현장 실천사항(표) → Ⅵ.확인학습 → Ⅶ.한 장 요약 → Ⅷ.참고자료. 대단원을 `Ⅰ.Ⅱ.Ⅲ.` 로마숫자로 써서 계획서(1.2.3.)와 구분되는 연수 교재 양식이며 **예산 항목 없음**. 개조식 + 명사형 종결 |
...
검색 도구를 지원하지 않는 모델은 400을 돌려주는데, 이때 모델 자체를 차단하면 최신 모델이 1시간 동안 배제되어 이후 다른 기능까지 구세대 모델로 생성된다. 그래서 도구가 거부되면 같은 모델로 도구 없이 한 번 더 시도한 뒤에야 다음 모델로 넘어간다.

웹 검색 그라운딩(`useSearchGrounding`)을 켠 요청은 `config.tools`에 `googleSearch`를 붙여 호출하고, 응답의 `groundingMetadata`에서 출처·검색어·검색 제안 위젯을 `groundingSources.ts`가 추려 `grounding` 필드로 함께 돌려준다. JSON 강제 출력(`responseJson`)과는 함께 쓸 수 없어 그때는 도구를 붙이지 않는다. 이 옵션은 연수자료에서 사용자가 켠 경우에만 전달되며 기본값은 꺼짐이다(검색 건수만큼 과금).
...
| 연수자료 | 연수 주제, 연수 대상, 추가 사항, 참고 파일 | 계획서와 다른 연수 교재 양식으로, 목표·내용·사례·실천·확인학습 중심 구성. 개조식 + 명사형 종결 | 연수자료 HTML (+ 웹 검색을 켠 경우 출처 목록) |
...
4. 공문서·품의서는 합쇼체, 계획서·보고서는 보고서체, 연수자료는 개조식 명사형 종결, 가정통신문은 안내문체를 적용한다.
5. 연수자료에서 웹 검색 참조를 켠 경우, 근거 규칙을 "지어내지 않기"에서 "검색으로 확인한 내용만 출처와 함께 쓰기"로 교체해 프롬프트를 구성한다.
