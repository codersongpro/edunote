...
3. 세부 운영 결과: 회차별/활동별 진행 내용 표 + 하단에 사진 첨부 표(2×2 또는 2×N 셀, 셀 안에 '[사진 첨부]'와 사진 설명 텍스트 삽입)
4. 만족도 조사 결과: 참여자 만족도 설문 결과 요약 표(항목·만족도·주요 의견) — 해당 정보가 없으면 '별도 조사 실시 예정'으로 기재
5. 예산 정산: [목|세목|산출내역|계획액|집행액|잔액|비고] 7열 표, 합계 행 포함, 집행률 명시
6. 운영 성과 및 제언: 구체적 성과(수치 포함) + 개선 사항 및 차기 계획 제언
...
2. 공문의 업무 주제를 교사가 한눈에 알 수 있는 짧은 문구로 추출하세요. 예: "AI 활용 수업 연수 신청", "학교자율시간 운영 자료 제출"
3. 핵심 내용을 5줄 이내로 요약하세요.
4. 해야 할 일을 체크리스트로 정리하세요.
5. 보낸 사람/시행 기관, 담당 부서, 담당자, 연락처가 있으면 원문 표현 그대로 추출하세요.
6. 마감일, 제출처, 제출 방법, 담당 부서, 필요한 붙임/서류가 있으면 별도 항목으로 정리하세요.
7. 날짜나 제출 기한이 불명확하면 "공문 원문 확인 필요"라고 표시하세요.
8. 출력은 아래 형식을 지켜주세요.
...
- 시작 화면에는 반드시 실제로 동작하는 <button> 요소의 "퀴즈 시작" 버튼을 넣고, JavaScript click 이벤트로 풀이 화면을 열어야 합니다.
...
${gradeGuidance ? `\n${gradeGuidance}\n` : ''}
[게임 요구사항]
- 게임 유형: 수업 주제에 가장 적합한 유형을 선택하세요
  (예: 낱말 맞추기, 용어-정의 매칭, 빈칸 채우기, 순서 배열, O/X 빠른 답, 카드 뒤집기 기억 게임, 미로 탈출, 타임어택 미션 등)
- 게임 내용: 해당 수업의 핵심 개념·용어·사실을 바탕으로 8~15개 문항 또는 게임 요소를 만드세요
- 난이도: ${params.grade}에 맞게 설정하세요
- 시작 화면에는 반드시 실제로 동작하는 <button> 요소의 "게임 시작" 버튼을 넣고, 버튼 클릭 전까지 게임 본 화면을 숨기세요
