...
3. 결과는 짧게 작성하세요. 전체 분석 결과가 20줄을 넘지 않게 하세요.
4. 핵심 요약은 최대 3개 bullet, 각 bullet은 1문장으로 작성하세요.
5. 해야 할 일은 실제 행동이 필요한 항목만 최대 5개 체크리스트로 정리하세요.
6. 보낸 사람/시행 기관, 담당 부서, 담당자, 연락처가 있으면 원문 표현 그대로 추출하세요.
7. 마감일, 제출처, 제출 방법, 필요한 붙임/서류가 있으면 별도 항목으로 정리하세요.
8. 날짜나 제출 기한이 불명확하면 "공문 원문 확인 필요"라고 표시하세요.
9. 원문 전체를 다시 쓰지 마세요. 원문은 앱에서 구분선 아래에 따로 표시합니다.
10. 출력은 아래 형식을 지켜주세요.
