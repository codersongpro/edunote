...
2. 전체 결과는 12줄 이내로 작성하세요.
3. 문장은 짧게 쓰고, 모두 개조식으로 정리하세요.
4. 실제 행동이 필요한 내용만 남기고 배경 설명은 과감히 줄이세요.
5. 마감일, 제출처, 제출 방법, 필요 서류가 있으면 반드시 적으세요.
6. 정보가 없거나 불명확하면 "원문 확인 필요"라고 쓰세요.
7. 원문 전체를 다시 쓰지 마세요.
8. 출력은 아래 형식을 지켜주세요.
...
- `;
  ...
  if (fileParts.length > 0) {
    return await aiGenerateMultipart(
      ...
      '당신은 학교와 교육청 공문을 짧은 업무 메모로 정리하는 행정 보조자입니다. 원문에 없는 사실을 만들지 말고, 마감과 제출 업무만 간결하게 드러내세요.',
      { temperature: 0.2 },
    );
  }
