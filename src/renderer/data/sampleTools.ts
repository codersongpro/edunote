...
잘한 점, 개선할 점, 격려의 말을 포함해서 친절하게 작성해줘.`,
  ...
  },
  {
    ...
    promptTemplate: `첨부된 이수증 이미지/파일에서 연수 정보를 추출하여 마크다운 표 하나로 정리해줘.
이수증이 여러 장이면 한 장씩 모두 읽어서 같은 표에 합쳐줘.
각 이수증에서 반드시 성명(이름)을 먼저 찾아서 첫 번째 열에 기록해줘.

| 성명 | 연수명 | 이수 날짜 | 이수 시간(h) | 연수 기관 | 연수번호 |
|------|--------|-----------|-------------|-----------|---------|

- 없는 항목은 '-'로 표시해줘.
- 결과는 마크다운 표 형식으로만 출력 (다른 설명 없이).
- 완성된 표는 성명 열을 기준으로 가나다 오름차순으로 정렬해줘.
`,
    createdAt: '2026-01-01T00:00:00.000Z',
    updatedAt: '2026-01-01T00:00:00.000Z',
  },
