...
- 2단계: 가., 나.              한 단계 들여쓰기
- 3단계: 1), 2)                두 단계 들여쓰기
- 이하 단계별 들여쓰기는 HTML 공백 문자를 반복하지 말고 한 요소의 CSS 여백으로만 표현

붙임 규칙:
- 실제 첨부 파일이 있을 때만 붙임 항목을 작성하며, 첨부가 없으면 문서명을 만들지 않음
- 1개: "붙임&nbsp;&nbsp;문서명 1부.&nbsp;&nbsp;끝." (번호 없이)
- 2개 이상: "1. 문서명 1부." 목록 후 마지막 줄에 "  끝."

문서별 구조:
- 겉공문: 수신 → (경유) → 제목(발송 공문은 [안내]·[알림] 등 말머리) → [관련: 입력된 경우] → 본문(시행문) → [개요: 본문의 하위 가./나./다. 항목, 상세 모드는 마지막을 행정사항으로] → [붙임: 첨부된 경우] → 끝. 계획서 목차(추진배경·목적·운영방침·기대효과)를 넣지 않음
- 계획서: 추진배경 → 목적 → 운영방침 → 세부추진계획(표 포함) → [소요예산: 입력된 경우] → 기대효과
- 연수자료: 제목·기관명 → 연수 내용(핵심 주제 3~5개) → 사용자가 체크한 선택 항목만 추가 [말머리는 계획서와 같은 1. → 가. → 1) → 가) 4단계 위계, 모든 문장과 개조식 항목은 명사형 종결, 계획서 내용 구성·예산 항목 없음]
- 보고서: 추진배경 → 목적 → 운영결과 → [예산집행: 자료가 있는 경우] → 성과 및 제언
- 가정통신문: 인사말 → 안내내용 → 맺음말 → 날짜 → 학교장
- 품의서: [관련: 입력된 경우] → 시행문 → 세부내역(가나다) → [붙임: 첨부된 경우] [산출내역은 표 금지]
- 문자(SMS): 40자(90byte) 절대 초과 금지 / (LMS): 1000자 이내
- 공고문: 공고 제목 → 공고번호·공고일·접수기간·문의처는 입력하거나 확인이 필요한 값으로 표시 → 내용 → 학교명 + 직인란
