...
import { generateCreativeActivityReport, parseAnnualPlanFromImages, parseAnnualPlanFromDocuments, parseStudentObservationFromFiles } from '../services/geminiService';
...
const CreativeActivityGenerator: React.FC<Props> = ({ schoolLevel }) => {
  ...
  // UI State for Individual Student Observation File Analysis
  ...
  const handleObservationFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const extractedText = await parseStudentObservationFromFiles(filesToAnalyze, '창의적 체험활동 특기사항 - 개별 특이사항');
      if (!extractedText.trim()) {
        notifyToast({ type: 'warning', title: '파일에서 관찰 내용을 분석하지 못했습니다.' });
        return;
      }

      const current = creativeState.activeStudents[creativeState.currentStudentIndex]?.additionalContext || '';
      handleIndividualContextChange(current ? `${current}\n\n${extractedText.trim()}` : extractedText.trim());
                ...
                <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 bg-white dark:bg-[#221E1B]">
                        <div className="max-w-4xl mx-auto space-y-8">
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    <label className="block text-sm font-bold text-[#44403C] dark:text-[#C4B8B0]">개별 관찰 내용 (선택)</label>
                                    <input
                                        type="file"
                                    ...
                                    <button
                                        ...
                                        className="px-3 py-1 bg-white dark:bg-[#2E2822] text-orange-600 dark:text-orange-400 rounded-lg text-xs font-bold shadow-sm border border-[#E7E5E4] dark:border-[#2E2822] hover:bg-[#FAF9F7] dark:hover:bg-[#3A332D] flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
